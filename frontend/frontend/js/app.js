// ===== FINPESSOAL – FUNÇÕES COMUNS v4 =====
// Estado global de mês/ano persiste via sessionStorage entre páginas
...
// Persiste entre navegações via sessionStorage.
// Toda a aplicação usa getMesGlobal() / getAnoGlobal().
// ══════════════════════════════════════════════════════════════════
...
// Atalhos que os JSs de página usam (antes eram getMesSelecionado/getAnoSelecionado)
...
// Preenche os selects de filtro da topbar (desktop) com o mês global
function preencherFiltros() {
  preencherMesAno('selMes','selAno', getMesGlobal(), getAnoGlobal());
}

// ══════════════════════════════════════════════════════════════════
// SIDEBAR – Avatar + Seletor de Mês Mobile
// ══════════════════════════════════════════════════════════════════

/** Injeta avatar + nome + email na sidebar */
function renderizarSidebarUser() {
  ...
  const avatarHtml = usuario.foto_url
    ? `<div class="sidebar-avatar"><img src="${usuario.foto_url}" alt="avatar"
          onerror="this.parentElement.innerHTML='${ini}'" /></div>`
...
/**
 * Injeta o seletor de Mês/Ano dentro da sidebar para uso no mobile.
 * O container #sidebarMesSel é inserido antes do <nav> nas páginas que têm filtro.
 * Em desktop fica oculto via CSS; no mobile aparece no lugar do seletor do topbar.
 */
function renderizarSidebarMesSel(callbackAoMudar) {
  ...
  // Guarda referência ao callback da página
...
function onSidebarMesChange() {
  ...
  // Chama o callback da página (ex: carregarDados, carregarReceitas…)
...
function renderDiagnostico(containerId, totalReceitas, totalGastos, saldo) {
  ...
  let classeBar = pct>80?'vermelho':pct>50?'amarelo':'';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  preencherFiltros();       // preenche selects do topbar com o mês global
