...
async function popularSelectsDespesas() {
  ...
  if (selCat)  selCat.innerHTML  = cats.length  ? cats.map(c => `<option value="${c.nome}">${c.nome}</option>`).join('') : '<option>Sem categorias</option>';
  if (selTipo) selTipo.innerHTML = tipos.length ? tipos.map(t => `<option value="${t.codigo}">${t.nome}</option>`).join('') : '<option>Sem tipos</option>';
...
function renderDespesas(lista) {
  ...
  el.innerHTML = lista.map(d => {
    ...
    <div class="item-card ${d.pago ? 'pago' : ''}">
      ...
      <div class="item-info">
        <div class="item-desc">${d.descricao}
          <span class="badge badge-${d.tipo}">${tipoLabel}</span>
          ${d.pago ? '<span class="badge badge-pago">Pago</span>' : ''}
        </div>
        <div class="item-sub">${d.categoria} · ${MESES[d.mes - 1]} ${d.ano}</div>
      </div>
      <div class="item-valor item-valor-neg">${formatarMoeda(d.valor)}</div>
      <div class="item-acoes">
        <button class="btn-acao ${d.pago ? '' : 'success'}"
                onclick="togglePago(${d.id},${!d.pago})"
                title="${d.pago ? 'Desfazer' : 'Pago'}">${d.pago ? '↩️' : '✅'}</button>
...
function abrirModal() {
  ...
  const selCat  = document.getElementById('despCategoria');
  const selTipo = document.getElementById('despTipo');
  if (selCat?.options.length)  selCat.selectedIndex  = 0;
  if (selTipo?.options.length) selTipo.selectedIndex = 0;
