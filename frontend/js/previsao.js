// ===== FINPESSOAL – PREVISÕES JS v4 =====

let graficoProjecao = null;
let _dadosSimulacao  = null;

const MESES_ABREV = ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'];

// ── Simulação principal ───────────────────────────────────────────
// Usa getMesGlobal() / getAnoGlobal() — nunca lê diretamente os selects
...
// ── Simulador de compra (re-renderiza gráfico sem nova chamada API) ─
...
// ── Renderiza todos os componentes com os dados da API ────────────
...
function renderGrafico(projecao, comSimulador) {
  ...
  const datasets = [
    ...
    { label: 'Despesas', data: despesas, backgroundColor: 'rgba(220,38,38,0.65)',  borderRadius: 6, borderSkipped: false, order: 1 },
    {
      ...
      borderWidth: 2.5, tension: 0.35, fill: true, order: 0
    }
  ];
...
document.addEventListener('DOMContentLoaded', () => {
  // Injeta seletor de mês na sidebar mobile
  renderizarSidebarMesSel(simular);
  // Simula com o mês do estado global
  setTimeout(simular, 150);
});
